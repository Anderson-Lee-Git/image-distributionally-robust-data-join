...
def sample_pairs(group_1: pd.DataFrame, group_2: pd.DataFrame,
                 label: int, num_pairs: int, columns: list):
    sub_frame = pd.DataFrame(columns=columns)
    sub_md_1 = group_1.loc[group_1["label"] == label]
    sub_md_2 = group_2.loc[group_2["label"] == label]
...
def main():
    ...
    total_classes = md["label"].unique()
    pairs_per_class = 1000
    pairs_md = pd.DataFrame(columns=["id_1", "id_2", "label_1", "label_2"])
    # for each class
    ...
    pairs_md.to_csv(config("CIFAR100_PAIRS_META_PATH"))
